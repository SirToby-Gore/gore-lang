...
* `to char code` -> `int`: an int of the unicode for the that char,
    e.g. `'a'.toCharCode()` -> `97`
* `is numeric` -> `bool`: checks if the char is a number, 
    e.g. `'7'.isInteger()` -> `true`,
    `'e'.isInteger()` -> `false`
* `is alpha` -> `bool`: checks if the char is in the alphabetical,
    e.g. `'a'.isAlpha()` -> `true`,
    `'5'.isAlpha()` -> `false`,
    `' '.isAlpha()` -> `false`
* `is punctuation` -> `bool`: checks if the char is punctuation,
    `'?'.isPunctuation()` -> `true`,
    `'p'.isPunctuation()` -> `false`
* `is whitespace` -> `bool`: checks if the char is whitespace,
    e.g. `' '.isWhitespace()` -> `true`,
    `'\t'.isWhitespace()` -> `true`,
    `'u'.isWhitespace()` -> `false`
...
* `substring`, `int`, ?`int` -> `string`: returns a string of the length, from the starting index, to ending index (the end of the string if not supplied),
    if the starting index is greater than the ending index then the string is sliced in a reversed order,
...
* `at position`, `int` -> `char`: returns the char at a position,
...
* `to double` -> `double`: returns the double representation of the integer,
    e.g. `7` -> `7.0`
* `to bool` -> `bool`: returns true if it is `1` or greater,
...
* `=` set equal to
* `.=` set equal to on a function e.g. `listOfRaffleTickets .= shuffle()`
